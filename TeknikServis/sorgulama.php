...
<head>
    ...
    <link href="css-js/sorgu2.css" type="text/css" rel="stylesheet">
            ...
            $sorgu=0;
                foreach ($liste as $row) {
                    if ($row["telefon"] == $tel) {
                        ...
                        if ($row["status"] == 0) {
                            echo "<td><form action='index.php' method='post'> <input name='feedback' value='".$row['feedback']."'>";
                            echo "<input class='btn' type='submit' value='Kaydet' name='save'><input value='".$row['id']."' name='id' hidden> </form>";
                            echo "</td>";
                        } else {
